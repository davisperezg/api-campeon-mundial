...
export const createPago: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    //Asignando tipo de dato a la variable acuenta v0.0.2
    if (!acuenta || Number(acuenta) < 0) {
      req.body.acuenta = 0;
      const calRestante =
    ...
    //Asignando tipo de dato a la variable acuenta v0.0.2
    if (stateRenta === true && Number(acuenta) <= 0) {
      return res.status(400).json({
        message: "Si el alumno está dejando dinero a cuenta, ingresar el monto",
      });
    }

    //Validando. Admin registra el pago compleo sin monto a cuenta v0.0.2
    if (stateRenta === true && Number(acuenta) === tramiteFound.costo) {
      return res.status(400).json({
        message: "Si el alumno está pagando completo, desmarque el check.",
      });
    }
    //Termina implementación validación v0.0.2
    if (stateRenta === false && Number(acuenta) > 0) {
      return res.status(400).json({
        message:
